...
let lastPromptHead = "";
  ...
  for (let i = 1; i <= MAX_TRIES; i++) {
    const hardSpec = `
[절대 규칙]
- 출력은 "대본 텍스트만" 출력한다. (설명/해설/요약/머리말 금지)
- 최소 ${MIN_SCRIPT_CHARS}자 이상. 절대 ${MIN_SCRIPT_CHARS}자 미만으로 끝내지 마라.
- 10~12분 분량(시니어가 천천히 읽는 속도 기준). 문장을 짧게 끊어 말하듯 자연스럽게.
- 너무 압축하지 말고, 각 항목은 최소 6~10문장 이상으로 충분히 풀어서 설명.
- 반드시 "사례 3개 이상" 포함 (실제 상황처럼 구체적으로: 나이/상황/결정/결과).
- 반드시 "체크리스트 10개" 포함 (번호 1~10).
- 반드시 "오늘 당장 할 행동 5개" 포함 (번호 1~5).
- 마지막은 "한 문장 결론"으로 끝낸다.

[대본 구조(반드시 이 순서)]
1) 콜드 오픈(0초 후킹): 손해/위험/숫자 포함 한 문장
2) 공감: 시니어가 겪는 현실 문제를 6문장 이상
3) 핵심 설명 1 (원리): 왜 이런 일이 생기는지
4) 핵심 설명 2 (돈/제도): 연금/보험/의료비/세금 중 최소 2개는 반드시 포함
5) 핵심 설명 3 (실수): 사람들이 가장 많이 하는 실수 3가지
6) 사례 3개 (각각 10문장 이상)
7) 체크리스트 10개
8) 오늘 당장 할 행동 5개
9) 마무리: 경고 1문장 + 확신 1문장 + 한 문장 결론

[문장 스타일]
- 너무 어려운 용어 금지. 초등학생도 이해할 정도로 풀어서.
- 단락마다 예시/비유 1개 이상.
- 숫자를 최소 10번 이상 사용(금액/기간/횟수/비교).
- 절대 1~2단락으로 끝내지 마라. 최소 25개 단락 이상으로 나눠 작성해라.
- 각 단락은 최소 4문장 이상으로 작성해라. (짧으면 실패)

다시 말한다: 반드시 ${MIN_SCRIPT_CHARS}자 이상이다. 미달이면 계속 이어서 작성해라.
`;
